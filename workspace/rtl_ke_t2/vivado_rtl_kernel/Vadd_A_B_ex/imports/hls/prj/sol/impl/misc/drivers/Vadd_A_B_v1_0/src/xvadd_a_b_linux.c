...
typedef struct {
    ...
    XVadd_a_b_uio_map maps[ MAX_UIO_MAPS ];
} XVadd_a_b_uio_info;

/***************** Variable Definitions **************************************/
static XVadd_a_b_uio_info uio_info;
...
static int uio_info_read_name(XVadd_a_b_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XVadd_a_b_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XVadd_a_b_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XVadd_a_b_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XVadd_a_b_Initialize(XVadd_a_b *InstancePtr, const char* InstanceName) {
	XVadd_a_b_uio_info *InfoPtr = &uio_info;
