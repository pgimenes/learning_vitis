...
int main(int argc, char** argv)
{
    ...
    for (cl_uint i = 0; i < number_of_words; i++) {
        if ((h_data[i] + 1) != h_A_output[i]) {
            printf("ERROR in Vadd_A_B::m00_axi - array index %d (host addr 0x%03x) - input=%d (0x%x), output=%d (0x%x)\n", i, i*4, h_data[i], h_data[i], h_A_output[i], h_A_output[i]);
            check_status = 1;
        }
    ...
    for (cl_uint i = 0; i < number_of_words; i++) {
        if ((h_data[i] + 1) != h_B_output[i]) {
            printf("ERROR in Vadd_A_B::m01_axi - array index %d (host addr 0x%03x) - input=%d (0x%x), output=%d (0x%x)\n", i, i*4, h_data[i], h_data[i], h_B_output[i], h_B_output[i]);
            check_status = 1;
        }
